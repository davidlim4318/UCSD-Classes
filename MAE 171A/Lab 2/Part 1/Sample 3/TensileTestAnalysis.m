...
% read .csv file
M = readmatrix("sample3.csv");

% isolate data
data = rmmissing(M(:,2:4));

% define measured cross-sectional area (m^2)
A = 2.86 * 12.44 * 10^-6;
...
ylabel("Force (kN)")
legend('Data','Location','southeast','FontSize',12)
ax = gca;
...
frameStart = round( length(strain) - data(end,1) / 10 + timeOffset / 10);
...
E = ( eps((1:n) + timeOffset) - eps(timeOffset + 1) ) \ ( sig((1:n) + timeOffset) - sig(timeOffset + 1) );
...
tiledlayout(1,2)
nexttile(1)
plot(0:length(strain(frameStart:end))-1,strain(frameStart:end),'b.','MarkerSize',10)
hold on
...
ylabel("Stress (Pa)")
legend('Data','Linear Least-Squares Approx.','0.2% Strain Offset Line','Yield Point','Ultimate Stress','Location','southeast','FontSize',14)
ax = gca;
ax.TitleHorizontalAlignment = 'left';
set(ax,'FontSize',16)
