...
float timeSeconds;  // current time in seconds

float A = 150;  // amplitude
float maxTime = 30;  // time limit for experiment in seconds (also defines max frequency in Hz)
...
void setup() 
{
  ...
  delay(3000);  // delay to allow for unplugging after reset
...
void loop()
{
  ...
  // sine function with amplitude A and increasing frequency (chirp-like signal that increases by 1 Hz every second)
  y = A*sin(PI*timeSeconds*timeSeconds);
  ...
  if (timeSeconds > maxTime) {
    analogWrite(pwmPin, 0);  // set to not be spinning
    Serial.println(output);
    while(1) {  // stop main loop
      Serial.println("Idle...");
      delay(1000);  
    }
  }
